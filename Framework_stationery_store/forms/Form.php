...
public static function form_select_price($pageID){
        $form='<form method="post" action="index.php?pageID='.$pageID.'">';
        $form.='<div class="form-group">';
        $form.='<label for="userID">Enter maximum price: </label><input required type="number" class="form-control" id="Price" name="Price" >';
        $form.='</div> ';
        $form.='<button type="submit" class="btn btn-default" value="TRUE" name="btnSearchPrice">Search</button>';
        $form.='</form>';
        return $form;
}    

public static function form_select_category($productCategoriesTable,$pageID){
    ...
    //prepare an array of product categories for the drop down list
        ...
        if($rs=$productCategoriesTable->select_all()){
            while ($row = $rs->fetch_assoc()){
                $productCategoriesList[$row['CategoryID']]=$row['CategoryName'];   //build an array of county
            }
        } 
    
        $form='<form method="post" action="index.php?pageID='.$pageID.'">';
        $form.='<label for="productCategoriesID">Product Category</label>';
        $form.='<select class="form-control" id="CategoryID" name="CategoryID">';
        ...
        $form.='</select><br>';
        $form.='<button type="submit" class="btn btn-default" name="btnSearchCategory" value="TRUE">Search</button>';
        $form.='</form>';
        return $form;
}

public static function form_select_brand($productBrandsTable,$pageID){
    
    //prepare an array of product categories for the drop down list
        $productBrandsList=array();
        if($rs=$productBrandsTable->select_all()){
            while ($row = $rs->fetch_assoc()){
                $productBrandsList[$row['BrandID']]=$row['BrandName']; 
            }
        } 
    
        $form='<form method="post" action="index.php?pageID='.$pageID.'">';
        $form.='<label for="productBrandID">Product Brand</label>';
        $form.='<select class="form-control" id="BrandID" name="BrandID">';
              $form.= '<option value="1" selected>Select a Product Brand</option>';
          foreach($productBrandsList as $key=>$value){
              $form.= '<option value="'.$key.'">'.$value.'</option>';  
          }
        $form.='</select><br>';
        $form.='<button type="submit" class="btn btn-default" name="btnSearchBrand" value="TRUE">Search</button>';
        $form.='</form>';
        return $form;
} 
...
public static function form_edit_account($countyTable,$userRecord, $pageID){
        ...
        $form.='<label for="idCounty">Address</label>';
...
public static function form_confirmAddToCart($pageID,$btnText,$choice){
        $form='<form method="post" action="index.php?pageID='.$pageID.'">';
        $form.='<button type="submit" class="btn btn-default" value='.$choice.' name="btnConfirm">'.$btnText.'</button>';
        $form.='</form>';
        return $form;
...
public static function form_confirmRemoveFromCart($pageID,$btnText,$choice){
        $form='<form method="post" action="index.php?pageID='.$pageID.'">';
        $form.='<button type="submit" class="btn btn-default" value='.$choice.' name="btnRemove">'.$btnText.'</button>';
        $form.='</form>';
        return $form;
}  

public static function form_emptyCart($pageID,$btnText,$choice){
        $form='<form method="post" action="index.php?pageID='.$pageID.'">';
        $form.='<button type="submit" class="btn btn-default" value='.$choice.' name="btnEmpty">'.$btnText.'</button>';
        $form.='</form>';
        return $form;
}

public static function form_createOrder($pageID,$btnText,$choice){
        $form='<form method="post" action="index.php?pageID='.$pageID.'">';
        $form.='<button type="submit" class="btn btn-default" value='.$choice.' name="btnOrder">'.$btnText.'</button>';
        $form.='</form>';
        return $form;
}

public static function form_confirmEnableSuspend($pageID,$btnText,$choice){
        $form='<form method="post" action="index.php?pageID='.$pageID.'">';
        $form.='<button type="submit" class="btn btn-default" value='.$choice.' name="btnEnableSuspend">'.$btnText.'</button>';
        $form.='</form>';
        return $form;
}
 ...
 * @return string String containing the generated form.
 */
public static function form_add_product($productCategoriesTable,$productBrandsTable, $pageID){

        //prepare an array of product categories for the drop down list
        $productCategoriesList=array();
        if($rs=$productCategoriesTable->select_all()){
            while ($row = $rs->fetch_assoc()){
                $productCategoriesList[$row['CategoryID']]=$row['CategoryName'];   //build an array of categories          
            }
        } 
        
        //prepare an array of product brands for the drop down list
        $productBrandsList=array();
        if($rs=$productBrandsTable->select_all()){
            while ($row = $rs->fetch_assoc()){
                $productBrandsList[$row['BrandID']]=$row['BrandName'];   //build an array of brands
            }
        } 
        ...
        $form.='<label for="prodCode">Product Code</label><input required type="text" class="form-control" id="prodCode" name="prodCode" pattern="[0-9]{3}" title="Three number Product Code">';
        $form.='<label for="prodName">Product Name</label><input required type="text" class="form-control" id="prodName" name="prodName" pattern="[a-zA-Z0-9óáéí\' ()\- ]{1,45}" title="Product Name">';

        //form input drop down list (Select) for Brands
        $form.='<label for="prodBrand">Product Brand</label>';
        $form.='<select class="form-control" id="prodBrand" name="prodBrand">';
              $form.= '<option value="1" selected>Select a Product Brand</option>';
          foreach($productBrandsList as $key=>$value){
              $form.= '<option value="'.$key.'">'.$value.'</option>';  
          }
        ...
        $form.='<label for="prodCategory">Product Category</label>';
        $form.='<select class="form-control" id="prodCategory" name="prodCategory">';
              $form.= '<option value="1" selected>Select a Product Category</option>';
          foreach($productCategoriesList as $key=>$value){
              $form.= '<option value="'.$key.'">'.$value.'</option>';  
          }
        ...
        $form.='<label for="prodDescription">Product Description</label><input required type="text" class="form-control" id="prodDescription" name="prodDescription" pattern="[a-zA-Z0-9óáéí\' ()\- ]{1,500}" title="Product Description">';
        $form.='<label for="prodPrice">Product Price</label><input required type="text" class="form-control" id="prodPrice" name="prodPrice" pattern="^(\d{1,5}|\d{0,5}\.\d{1,2})$" title="Product Price - [2 Places Decimal]">';
        $form.='<label for="prodQuantityInStock">Quantity In Stock</label><input required type="text" class="form-control" id="prodQuantityInStock" name="prodQuantityInStock" value="0" pattern="[0-9]{1,5}" title="Product Quantity [INTEGER VALUE] in Stock (default =0)">';

        $form.='<label for="prodSale">Product On Sale?</label>';
        $form.='<select class="form-control" id="prodSale" name="prodSale">';
    ...
    public static function form_select_product($pageID){
        $form='<form method="post" action="index.php?pageID='.$pageID.'">';
        $form.='<div class="form-group">';
        $form.='<label for="productCode">Product Code</label><input required type="text" class="form-control" id="prodCode" name="prodCode" pattern="[a-zA-Z0-9\-]{1,11}">';
        $form.='</div> ';
...
public static function form_edit_product($productCategoriesTable, $productBrandsTable, $productRecord, $pageID){
        ...
        //form input fields for product code and description
        $form.='<label for="productCode">Product Code</label><input type="text" class="form-control" value="'.$ProdCode.'" id="prodCode" name="prodCode" pattern="[a-zA-Z0-9\-]{1,11}" >';
        $form.='<label for="productDescription">Product Name</label><input required type="text" class="form-control" value="'.$ProdName.'" id="prodName" name="prodName" pattern="[a-zA-Z0-9óáéí\'()\- ]{1,45}" title="Product Description">';
        $form.='<label for="productDescription">Product Description</label><input required type="text" class="form-control" value="'.$ProdDescription.'" id="prodDescription" name="prodDescription" pattern="[a-zA-Z0-9óáéí\'()\- ]{1,45}" title="Product Description">';

        //form input drop down list (Select) for categories
        $form.='<label for="productCategoriesID">Product Category</label>';
        $form.='<select class="form-control" id="prodCategory" name="prodCategory">';
              $form.= '<option value="'.$CategoryID.'" selected>'.$productCategoriesList[$CategoryID].'</option>';
          foreach($productCategoriesList as $key=>$value){
              $form.= '<option value="'.$key.'">'.$value.'</option>';  
          }
        $form.='</select>';
      ...
      //form input drop down list (Select) for Brands
        $form.='<label for="prodBrand">Product Brand</label>';
        $form.='<select class="form-control" id="prodBrand" name="prodBrand">';
              $form.= '<option value="'.$BrandID.'" selected>'.$BrandName.'</option>';
          foreach($productBrandsList as $key=>$value){
              $form.= '<option value="'.$key.'">'.$value.'</option>';  
          }
        $form.='</select>';
        ...
        $form.='<label for="prodPrice">Product Price</label><input required type="text" class="form-control" value="'.$ProdPrice.'" id="prodPrice" name="prodPrice" pattern="^(\d{1,5}|\d{0,5}\.\d{1,2})$" title="Product Price - [2 Places Decimal]">';
        $form.='<label for="prodQuantityInStock">Quantity In Stock</label><input required type="text" class="form-control" value="'.$ProdQuantityInStock.'" id="prodQuantityInStock" name="prodQuantityInStock" value="0" pattern="[0-9]{1,5}" title="Product Quantity [INTEGER VALUE] in Stock (default =0)">';
