...
class UserTable extends TableEntity {
    ...
    public function getRecordByID($userID){ 
        ...
        $this->SQL="SELECT u.UserNr,
                    ...
                    u.userEnabled,
                    u.pointBalance 
                    FROM user u,usertype ut WHERE u.userID = '$userID' AND u.userTypeNr = ut.userTypeNr";
    ...
    public function getRecordByIDStatus($userID){ 
     ...
     */
     public function getAllRecords($userType=0){
        ...
        if($userType){ //select data for the specified user type
           $this->SQL = "SELECT userID,FirstName,LastName, ut.userTypeDescr AS UserType, userEnabled AS LoginEnabled, pointBalance as Points FROM user u,usertype ut where u.userTypeNr=ut.userTypeNr and u.userTypeNr=$userType ORDER BY userTypeDescr";
        } 
        else{  //select data for all user types
           $this->SQL = "SELECT userID,FirstName,LastName, ut.userTypeDescr AS UserType, userEnabled AS LoginEnabled, pointBalance as Points FROM user u,usertype ut where u.userTypeNr=ut.userTypeNr ORDER BY userTypeDescr";
    ...
    public function addRecord($postArray,$encryptPW){
        ...
        $this->SQL="INSERT INTO user (userID,FirstName,LastName,PassWord,email,mobile,idcounty,userTypeNr,userEnabled,pointBalance) "
                . "VALUES ('$userID','$firstName','$lastName','$password','$email','$mobile',$idCounty,2,1,0)";   
    ...
    public function updateRecord($postArray, $userID){
        ...
        $this->SQL="UPDATE user SET FirstName='$firstName',LastName='$lastName',mobile='$mobile',idcounty=$idCounty WHERE userID='$userID'";   
    ...
    public function getUserStatus($userID){
         $this->SQL = "SELECT userEnabled FROM user where userID = '$userID'";
         ...
         try{
            ...
            if($rs){
                if($rs->num_rows){  //this query should return at least 1 record
                    $row = $rs->fetch_assoc();
                    return $row["userEnabled"];  //the resultset can be returned as it contains ONLY one record
                }
                else{
                    //no records returned for this query 
                    return false;
   ...
   public function updateStatus($userID, $userEnabled){
     
       
       //if user enabled
       if ($userEnabled){
       
       $this->SQL="UPDATE user SET userEnabled=0 WHERE userID='$userID'"; 
       }
       else
       {
          
           $this->SQL="UPDATE user SET userEnabled=1 WHERE userID='$userID'"; 
       }
        ...
        //execute the UPDATE query
            ...
            //check the update query was successful
            if ($this->db->affected_rows===1 && $rs){return TRUE;}else{
                
                return FALSE;}
       }
       
       public function getUserPointBalance($userID){
         $this->SQL = "SELECT pointBalance FROM user where userID = '$userID'";
         ...
         try{
            ...
            if($rs){
                if($rs->num_rows){  //this query should return at least 1 record
                    $row = $rs->fetch_assoc();
                    return $row["pointBalance"];  //the resultset can be returned as it contains ONLY one record
                }
                else{
                    //no records returned for this query 
                    return false;
    ...
    public function updateUserPointBalance($userID){
     
       
       $this->SQL="UPDATE user SET pointbalance = pointbalance + 10 WHERE userID='$userID'"; 
