...
Class HelperHTML {
    ...
    */
    ...
    public static function generateAddToCartTABLE($resultSet, $pageID, $code){
        ...
        while ($row = $resultSet->fetch_assoc()) {  //fetch associative array
            ...
            {   $table.=  '<tr>';
                foreach($row as $key=>$value){
                    if(!($key===$code)){ //hide the primary key column
                        $table.=  "<th>$key</th>"; //echo the keys as table headings for the first row of the HTML table
                    }
                }
                $table.=  "<th>Cart</th>";
            ...
            if ($key===$code){
                $table.=  '<td>';
                $table.=Form::form_confirmAddToCart($pageID, 'Add to Cart',$row[$code]);
                $table.=  '</td>';
            $table.=  '</tr>';
            }
            }
    ...
    public static function generateSelectTABLEUSERSTATUS($resultSet,$selectKeyField,$pageID,$buttonText){
        //This STATIC method returns a HTML table as a string
        //It takes one argument - $resultSet which must contain an object
        //of the $resultSet class
        $table="";  //start with an empty string
        ...
        while ($row = $resultSet->fetch_assoc()) {  //fetch associative array
            ...
            foreach($row as $key=>$value){ //generate the selector button with the hidden key value
                if ($key===$selectKeyField){
                    $table.=  "<td>";
                    $table.=Form::form_confirmEnableSuspend($pageID, 'Enable/Suspend',$row[$selectKeyField]);    
                    $table.=  '</td>';
                }
                }                
    ...
    public static function generateRemoveFromCartTABLE($resultSet, $pageID, $code){
        ...
        while ($row = $resultSet->fetch_assoc()) {  //fetch associative array
            ...
            if ($key===$code){
                $table.=  '<td>';
                $table.=Form::form_confirmAddToCart($pageID,'Remove',$row[$code]);
                $table.=  '</td>';
            $table.=  '</tr>';
            }
            }
    ...
    public static function generateAddToWishlistTABLE($resultSet, $pageID, $code){
        ...
        while ($row = $resultSet->fetch_assoc()) {  //fetch associative array
            ...
            if ($key===$code){
                $table.=  '<td>';
                $table.=Form::form_confirmAddToCart($pageID, 'Add to Wishlist',$row[$code]);
                $table.=  '</td>';
            $table.=  '</tr>';
            }
            }
