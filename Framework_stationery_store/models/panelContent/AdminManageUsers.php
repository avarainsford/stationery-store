    ...
    public function setPanelContent_1(){           
        switch ($this->pageID) {
            ...
            case "viewUsers":
                ...
                if ($rs){                   
                    $this->panelContent_1=HelperHTML::generateSelectTABLEUSERSTATUS($rs, 'userID' ,$this->pageID,'Enable/Suspend User');
                    
                    if(isset($this->postArray['btnEnableSuspend'])){
                    
                    $customerId = $this->postArray['btnEnableSuspend'];
                    $userEnabled=$userTable->getUserStatus($customerId);
                    if($userTable->updateStatus($customerId, $userEnabled)){
                        $this->panelContent_1='User status changed successfully';  //record successfully deleted
                            }
                            else{
                                $this->panelContent_1='Unable to change users status'; 
                            }
                }    
                }
                else{
                    $this->panelContent_1="No user data available"; 
                }
                break;               
            case "editUsers":
                ...
                if(isset($this->postArray['btnUserSelect'])){        //The Select user button has been pressed   
                    ...
                    if($rs=$userTable->getRecordByIDStatus($this->postArray['userID'])){ //a valid user ID is entered    
                        $this->panelContent_1.=HelperHTML::generateSelectTABLEUSERSTATUS($rs, 'userID' ,$this->pageID,'Enable/Suspend User');
                        $this->panelContent_1.=Form::form_confirm($this->pageID, 'Confirm Record Deletion',$this->postArray['userID']);
                    }
                    else{ //user ID entered is not a valid ID in the user table
                        $this->panelContent_1='The user ID entered  is not valid.';
                    }
                }
                
                
                elseif(isset($this->postArray['btnConfirm'])){
                        $this->panelContent_1='The confirm button is pressed -'.$this->postArray['btnConfirm'];
                        //try to delete the record and confirm if successful
                        if($userTable->deleteRecordbyID($this->postArray['btnConfirm'])){  
                            $this->panelContent_1='User record deleted';  //record successfully deleted
                            }
                            else{
                                $this->panelContent_1='Unable to delete user record'; //Unable to delete user record - there may be some dependencies
                            }     
                }
                ...
                if(isset($this->postArray['btnEnableSuspend'])){
                    ...
                    if($userTable->updateStatus($customerId, $userEnabled)){
                        $this->panelContent_1='User status changed successfully';  //record successfully deleted
                                ...
                                $this->panelContent_1='Unable to change users status'; //Unable to delete user record - there may be some dependencies
                            }
                }
...
//                     $userTypeTable=new UserTypeTable($this->db);
                     ...
                     if($userRecord){  //record found - generate an edit form
                         $this->panelContent_1.=HelperHTML::generateSelectTABLEUSERSTATUS($userRecord, 'userID' ,$this->pageID,'Enable/Suspend User');
                         echo '<br>';
                         $this->panelContent_1.=Form::form_confirm($this->pageID, 'Confirm Record Deletion',$this->postArray['userID']);
        
                     }
                     else{ //no record for this ID
                         $this->panelContent_1='No record found with the following user ID : '.$userID;
                     }
                }
